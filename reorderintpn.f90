...
!
!  modified by CWJ @ SDSU Mar 2022 for pn
!
module information
    implicit none
...
print*,' '
print*,' Program to reorder single-particle orbits in  PN-formatted .int files '
print*,' '
print*,' (INDUCED CORE ENERGY NOT YET FINISHED )'
...
subroutine open_sps_file(ichar)
	...
	read(1,*)norb
	if(ichar=='i')then
		allocate(orbqni(norb*2))
		curorbqn=> orbqni
		norbiti = norb
	else
		allocate(orbqnf(norb*2))
		curorbqn=> orbqnf
		norbitf = norb
	end if
	do iorb =1,norb
		read(1,*)xn,xl,xj,w
		! START with PROTON ORBITALS
		...
		! NOW ADD NEUTRON ORBITALS
		curorbqn(iorb+norb)%nr = nint(xn)
		curorbqn(iorb+norb)%l  =  nint(xl)
		curorbqn(iorb+norb)%j = nint(xj*2)
        curorbqn(iorb+norb)%map = 0         ! initialize
		curorbqn(iorb+norb)%w   = w        ! used to determine core
		curorbqn(iorb+norb)%core = .false.   ! not always used
	...
	do orbf = 1,norbitf
		...
		do orbi = 1,norbiti
			...
			if(orbqni(orbi)%nr ==orbqnf(orbf)%nr  .and.  & 
				  ...
				  orbqni(orbi+norbiti)%map = orbf+norbitf    ! DUPLICATE FOR NEUTRONS
				  orbqnf(orbf+norbitf)%map = orbi+norbiti
...
!--------- READ PAST HEADER-------	
	...
	allocate(spe(norbiti*2))
	read(3,*)ntbmei,(spe(i),i=1,min(norbiti*2,10))
	
	if(norbiti>10)then
		k = 10
		do while(k < norbiti*2)
			read(3,*)(spe(k+i),i=1,min(norbiti*2-k,10))
			k = k+10
...
subroutine writehamiltonian
	...
	write(33,'(i8,10f9.4)')ntbmef,(spe(orbqnf(j)%map),j=1,min(10,norbitf*2))
	print*,norbitf,' final s.p. orbits '
	if(norbitf > 10)then
        nline = norbitf*2/10
        nres = norbitf*2-nline*10
		print*,nline,nres
        if(nline > 1)then
           do j = 2,nline
		...
		print*, ' Testing sum rule ', nres+nline*10,norbitf*2
